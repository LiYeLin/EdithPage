...
export function LiquidDragLayer({ visual, portalStyle }: { visual: LiquidVisual; portalStyle: CSSProperties }) {
  ...
  useEffect(() => {
    ...
    const sourceBubble = visual.source.variant === 'module' ? findBubble(templateRoot.current, visual.source.moduleId) : null
    const sourceTile = findSiteTile(templateRoot.current, visual.source)
    ...
    const tick = (now: number) => {
      ...
      if (visual.targetModuleId !== previousTarget) {
        ...
        targetBubble = previousTarget ? findBubble(templateRoot.current, previousTarget) : null
      }
      ...
      if (lifetime.dragging) {
        ...
        if (success && (!arrivalTile || !arrivalTile.isConnected) && lifetime.pending) arrivalTile = findSiteTile(templateRoot.current, lifetime.pending)
  ...
  }, [templateRoot, visual, lifetime, x, y, source, target, scale, opacity, iconScale])
