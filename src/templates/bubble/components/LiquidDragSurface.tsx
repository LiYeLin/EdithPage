...
import { clamp, contact, mix, springStep } from '../physics/liquid'
...
export function LiquidDragSurface({ visual }: { visual: LiquidVisual }) {
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      const source = visual.source.variant === 'module' ? findBubble(templateRoot.current, visual.source.moduleId) : null
      // Preview adhesion before dnd-kit declares a drop target; never change business hit testing.
      const target = visual.targetModuleId === visual.source.moduleId ? null : visual.targetModuleId ? findBubble(templateRoot.current, visual.targetModuleId) : [...(templateRoot.current?.querySelectorAll<HTMLElement>('.bubble-shell') ?? [])].find(shell => {
