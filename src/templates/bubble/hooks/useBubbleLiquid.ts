...
export function useBubbleLiquid(enabled: boolean) {
  ...
  useEffect(() => {
    ...
    const punchTarget = skin.closest<HTMLElement>('.bubble-punch-body')
    let punchAnimation: Animation | null = null
    const splash = () => {
    ...
    const punch = (event: PointerEvent) => {
      ...
      const travel = Math.min(18, Math.max(9, distance * .18))
      punchAnimation?.cancel()
      punchAnimation = punchTarget?.animate(
        { transform: [
          'translate3d(0, 0, 0)',
          `translate3d(${directionX * travel}px, ${directionY * travel}px, 0)`,
          `translate3d(${directionX * travel * .48}px, ${directionY * travel * .48}px, 0)`,
          'translate3d(0, 0, 0)',
        ] },
