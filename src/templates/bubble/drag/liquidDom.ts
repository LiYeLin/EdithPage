...
export function findSiteTile(root: ParentNode | null, arrival: Arrival): HTMLElement | null {
  return [...(root?.querySelectorAll<HTMLElement>('.site-tile-wrap') ?? [])].find((tile) =>
    tile.dataset.siteId === arrival.siteId && tile.dataset.moduleId === arrival.moduleId && tile.dataset.variant === arrival.variant) ?? null
}

export function findBubble(root: ParentNode | null, moduleId: string): HTMLElement | null {
  return [...(root?.querySelectorAll<HTMLElement>('.module-panel') ?? [])]
