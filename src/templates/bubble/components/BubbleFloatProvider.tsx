...
export function BubbleFloatProvider({ blocked, children }: { blocked: boolean; children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const start = () => {
      idleTimer = 0
      if (document.hidden || templateRoot.current?.querySelector('.bubble-float-body:hover, .bubble-float-body:focus-within')) return
  ...
  }, [blocked, desktopMotionAllowed, templateRoot])
